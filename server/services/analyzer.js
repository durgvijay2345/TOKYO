// ============================================================
// TOKYO PULSE — Analysis Engine (Orchestrator)
// Delegates to AI module for classification, scoring, insights
// ============================================================
...
async function analyze(repoData) {
  ...
  // 1. Run Gemini Batch Analysis on the most detailed top 50 commits to save latency
  ...
  if (detailedCommits.length > 0) {
    ...
    if (geminiResults) {
      ...
      console.warn(`⚠️ Warning: LLM analysis failed. Check API key. Falling back to empty defaults.`);
    }
  }

  const profiles = await analyzeContributions(commits, contributors, pullRequests, geminiResults);

  // Enrich profiles with data from /stats/contributors endpoint
  // (gives per-author additions/deletions without needing individual commit details)
  ...
  // Build commit summaries strictly with AI classification
